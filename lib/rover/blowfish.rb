...
class Blowfish
  ...
  def underscore_f input
    input = to_uint32 input
    h = to_uint32(@salt[0][input >> 24] + @salt[1][input >> 16 & 0xff])
    to_uint32(( h ^ @salt[2][input >> 8 & 0xff] ) + @salt[3][input & 0xff] )
  end

  def to_uint32 value
    value = emulate_overflow(value)
    unsigned_binary_input =  "0" + value.to_s(2)
    unsigned_binary_input.to_i(2).to_s(10).to_i
  end

  def emulate_overflow(x)
    x = x << 32
